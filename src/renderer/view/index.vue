<template>
  <div id="wrapper">
    ...
    <div class="doc">
      ...
      <button @click="calc">开始计算</button><br><br>
      <ul>
        <li>
          <span>公司ID </span>
        ...
        <li v-for="(data, key) in dataMap" :key="key">
          <span>{{key}}</span>
          <span>{{data.k4_average}}</span>
...
export default {
  ...
  methods: {
    calc() {
      ...
      csv({ delimiter: ";" })
        .fromStream(fs.createReadStream(path.join(__static, 'calc/data.csv')))
        .then(jsonArray => {
          // 将数据按公司分类
          for (let i = 0, len = jsonArray.length; i < len; i++) {
            ...
            if (companyDataMap.has(json.id)) {
              ...
              companyDataMap.set(json.id, {
                nindnme: json.Nindnme,
                data: [json]
              });
            }
          }
            ...
            for (let i = 0, len = value.data.length; i < len; i++) {
              ...
              if (!isNaN(row.earning1) && !isNaN(row.returnSe)) {
                if (
                  ...
                  row.earning1 = f(row.earning1) / SCALE;
                  x.push(row.returnSe);
                  y.push(row.earning1);
                  reserveArr.push(row);
                }
